...
builder.Services.AddSingleton<CosmosClient>((_) =>
{
    CosmosClient client = new(
        connectionString: builder.Configuration["CosmosDB:ConnectionString"] ?? builder.Configuration["CosmosDB_ConnectionString"]
    );
    return client;
});
...
//{
//    var endpoint = new Uri(builder.Configuration["AzureOpenAI:Endpoint"] ?? builder.Configuration["AzureOpenAI_Endpoint"]);
//    var credentials = new AzureKeyCredential(builder.Configuration["AzureOpenAI:ApiKey"] ?? builder.Configuration["AzureOpenAI_ApiKey"]);
...
builder.Services.AddSingleton<Kernel>((_) =>
{
    ...
    #pragma warning disable SKEXP0010 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
       kernelBuilder.AddAzureOpenAITextEmbeddingGeneration(
           deploymentName: builder.Configuration["AzureOpenAI:EmbeddingDeploymentName"] ?? builder.Configuration["AzureOpenAI:EmbeddingDeploymentName"],
           endpoint: builder.Configuration["AzureOpenAI:Endpoint"] ?? builder.Configuration["AzureOpenAI:Endpoint"],
           apiKey: builder.Configuration["AzureOpenAI:ApiKey"]??builder.Configuration["AzureOpenAI:ApiKey"]
       );
